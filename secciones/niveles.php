...
?> 
<!-- Boton nuevo  -->
<div id="buton_nuevo" class="btn btn-secondary" data-titulo="Agregar nuevo nivel"><i class="fas fa-plus"></i> Nuevo</div>
<!-- Tabla de registros  -->
<table id="tabla_niveles" class="table table-striped" style="width:100%">
    <thead>
        <tr>
            <th>ID</th>
            <th>Descripción</th>
    ...
    <tbody>
        <?php foreach($get__response->niveles as $item){ ?>   
            <tr>
                <td><?= $item->idNivel ?></td>
                <td><?= $item->descripcion ?></td>
                <td class="td_botones">
                    <div class="btn btn-light" id="buton_editar"    data-titulo="Editar el nivel <?= $item->idNivel ?>"                           data-id="<?= $item->idNivel ?>" data-descripcion="<?= $item->descripcion ?>"><i class="fas fa-edit"></i> Editar</div>
                    <div class="btn btn-light" id="buton_eliminar"  data-mensaje="Realmente desea eliminar nivel con el ID <?= $item->idNivel ?>" data-id="<?= $item->idNivel ?>"><i class="fas fa-trash-alt"></i> Eliminar</div>
                </td>
            </tr>
        <?php } ?>
    </tbody>
