...
?> 
<!-- Boton nuevo  -->
<div id="buton_nuevo" class="btn btn-secondary" data-titulo="Agregar nueva modalidad"><i class="fas fa-plus"></i> Nuevo</div>
<!-- Tabla de registros  -->
<table id="tabla_modalidad" class="table table-striped" style="width:100%">
    <thead>
        <tr>
            <th>ID</th>
            <th>Descripción</th>
    ...
    <tbody>
        <?php foreach($get__response->modalidades as $item){ ?>   
            <tr>
                <td><?= $item->idModalidad ?></td>
                <td><?= $item->descripcion ?></td>
                <td class="td_botones">
                    <div class="btn btn-light" id="buton_editar"    data-titulo="Editar la modalidad <?= $item->idModalidad ?>"                           data-id="<?= $item->idModalidad ?>" data-descripcion="<?= $item->descripcion ?>"><i class="fas fa-edit"></i> Editar</div>
                    <div class="btn btn-light" id="buton_eliminar"  data-mensaje="Realmente desea eliminar modalidad con el ID <?= $item->idModalidad ?>" data-id="<?= $item->idModalidad ?>"><i class="fas fa-trash-alt"></i> Eliminar</div>
                </td>
            </tr>
        <?php } ?>
    </tbody>
