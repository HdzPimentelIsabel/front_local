...
?> 
<!-- Boton nuevo  -->
<div id="buton_nuevo" class="btn btn-secondary" data-titulo="Agregar nuevo estado civil"><i class="fas fa-plus"></i> Nuevo</div>
<!-- Tabla de registros  -->
<table id="tabla_edoCiviles" class="table table-striped" style="width:100%">
    <thead>
        <tr>
            <th>ID</th>
            <th>Descripción</th>
    ...
    <tbody>
        <?php foreach($get__response->edoCiviles as $item){ ?>   
            <tr>
                <td><?= $item->idEdoCivil ?></td>
                <td><?= $item->descripcion ?></td>
                <td class="td_botones">
                    <div class="btn btn-light" id="buton_editar"    data-titulo="Editar el estado civil <?= $item->idEdoCivil ?>"                           data-id="<?= $item->idEdoCivil ?>" data-descripcion="<?= $item->descripcion ?>"><i class="fas fa-edit"></i> Editar</div>
                    <div class="btn btn-light" id="buton_eliminar"  data-mensaje="Realmente desea eliminar estado civil con el ID <?= $item->idEdoCivil ?>" data-id="<?= $item->idEdoCivil ?>"><i class="fas fa-trash-alt"></i> Eliminar</div>
                </td>
            </tr>
        <?php } ?>
    </tbody>
